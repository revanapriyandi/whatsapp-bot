...
export const message = async (client: WAClient, msg: WAMessage): Promise<MessageSerialize> => {
  ...
  if (m.message) {
    m.key = msg.key
    m.id = m.key.id
    m.isBotMsg = (m.id.startsWith("BAE5") && m.id.length === 16) || (m.id.startsWith("3EB0") && m.key.id.length === 12) || (m.id.startsWith("ORIONPRO") && m.id.length === 20)
    ...
    m.sender = client.decodeJid(m.fromMe ? client.user.id : m.isGroupMsg || m.from === "status@broadcast" ? m.key.participant || msg.participant : m.from)
  ...
  m.groupMetadata = m.isGroupMsg && ((await database.getGroupMetadata(m.from)) ?? (await database.createGroupMetadata(m.from, (await client.groupMetadata(m.from)) as unknown)))
  ...
  if (m.quoted.message) {
    m.quoted.key = {
      participant: client.decodeJid(m.message[m.type]?.contextInfo?.participant),
      remoteJid: m?.message[m.type]?.contextInfo?.remoteJid || m.from || m.sender,
      fromMe: client.decodeJid(m.message[m.type].contextInfo.participant) === client.decodeJid(client.user.id),
      id: m.message[m.type].contextInfo.stanzaId
    }
    m.quoted.id = m.quoted.key.id
    m.quoted.isBotMsg = (m.quoted.id.startsWith("BAE5") && m.quoted.id.length === 16) || (m.id.startsWith("3EB0") && m.key.id.length === 12) || (m.quoted.id.startsWith("ORIONPRO") && m.quoted.id.length === 20)
    m.quoted.isGroupMsg = m.quoted.key.remoteJid.endsWith("g.us")
    m.quoted.from = client.decodeJid(m.quoted.key.remoteJid)
