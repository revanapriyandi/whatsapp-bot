...
export const formatDurasi = (duration: number): string => {
  const minutes = Math.floor(duration / 60);
  const seconds = duration % 60;

  let formattedDuration = '';
  if (minutes > 0) {
    formattedDuration = `${minutes.toString().padStart(2, '0')} menit`;
  }

  if (seconds > 0) {
    formattedDuration += ` ${seconds.toString().padStart(2, '0')} detik`;
  }

  return formattedDuration;
}

export const formatAngka = (number: number): string => {
  let formattedNumber = '';

  if (number >= 1000000000) {
    formattedNumber = `${Math.floor(number / 1000000000)} milyar`;
  } else if (number >= 1000000) {
    formattedNumber = `${Math.floor(number / 1000000)} juta`;
  } else {
