...
export const execute = async (client: WAClient, call: CallSerialize): Promise<unknown> => {
  if (!call.callFrom.endsWith("g.us") && call.status === "offer" && !config.ownerNumber.includes(call.callFrom.replace(/\D+/g, ""))) {
    ...
    try {
      if (config.antiCall.reject) {
        await client.rejectCall(call.callId, call.callFrom)

        await call.reply(i18n.translate("handlers.call.reject", {}, user.language))
      }

      if (config.antiCall.block) {
        await client.updateBlockStatus(call.callFrom, "block")
      ...
      return client.log(`${color.hex("#940c9c" as HexColor)("[CALL]")} ${color.cyan(`>> [${call.callId.length}]`)} from ${color.blue(user?.name || call.callFrom)}`.trim(), "info", Date.now())
    } catch {
      return client.log(`${color.hex("#940c9c" as HexColor)("[CALL]")} ${color.cyan(`>> [${call.callId.length}]`)} from ${color.blue(user?.name || call.callFrom)}`.trim(), "error", Date.now())
    }
  }
}
