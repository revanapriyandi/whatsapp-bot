...
export const execute = async (client: WAClient, message: MessageSerialize): Promise<unknown> => {
  ...
  const user = message.sender && ((await getUser(message.sender)) ?? (await createUser(message.sender, { name: message.pushname, ramadhan: false })))
  const isOwner = message.sender && (config.self ? config.ownerNumber.concat([client.decodeJid(client.user.id).split("@")[0]]) : config.ownerNumber).includes(message.sender.replace(/\D+/g, ""))
  ...
  const isBotGroupAdmin = message.isGroupMsg && !!groupAdmins.find((member) => member.id === client.decodeJid(client.user.id))
  ...
  setImmediate(() =>
    events.forEach(
      (event, key) =>
        ...
        event
          .execute({ client, message, command: cmd, prefix, args, arg, isGroupOwner, isGroupAdmin, isBotGroupAdmin, isOwner, user, group })
          .catch(() => client.log(`${color.red("[ERRS]")} ${color.cyan(`${key} [${message.body.length}]`)} from ${color.blue(user.name)} ${message.isGroupMsg ? `in ${color.blue(message.groupMetadata.subject || "unknown")}` : ""}`.trim(), "error", message.timestamps))
    )
  )
  ...
  if (command) {
    ...
    if (cooldowns.has(message.sender + cmd)) {
      ...
      return client.log(`${color.yellow("[SPAM]")} ${color.cyan(`${cmd} [${arg.length}]`)} from ${color.blue(message.pushname)} ${message.isGroupMsg ? `in ${color.blue(message.groupMetadata.subject || "unknown")}` : ""}`.trim(), "warning", message.timestamps)
    }
    ...
    try {
      ...
      typeof command.execute === "function" &&
        (await commandQueues.add(() => command.execute({ client, message, command: cmd, prefix, args, arg, isGroupOwner, isGroupAdmin, isBotGroupAdmin, isOwner, user, group }), {
          priority: isOwner ? 30 : user.role === "vip" ? 20 : user.role === "premium" ? 10 : 0
        }))
      await client.sendMessage(message.from, { react: { text: "✅", key: message.key } }, { quoted: message, ephemeralExpiration: message.expiration })
      return client.log(`${color.green("[CMDS]")} ${color.cyan(`${commands.findKey((v) => v === command)} [${arg.length}]`)} from ${color.blue(user.name)} ${message.isGroupMsg ? `in ${color.blue(message.groupMetadata.subject || "unknown")}` : ""}`.trim(), "success", message.timestamps)
    } catch (e: unknown) {
      await client.sendMessage(message.from, { react: { text: "❌", key: message.key } }, { quoted: message, ephemeralExpiration: message.expiration })
      ...
      return client.log(`${color.red("[ERRS]")} ${color.cyan(`${commands.findKey((v) => v === command)} [${arg.length}]`)} from ${color.blue(user.name)} ${message.isGroupMsg ? `in ${color.blue(message.groupMetadata.subject || "unknown")}` : ""}`.trim(), "error", message.timestamps)
    }
  }
  ...
  if (message.body.startsWith(">>") && isOwner) {
    new Promise((resolve, reject) => {
      ...
      .finally(() => client.log(`${color.purple("[EVAL]")} ${color.cyan(`>> [${arg.length}]`)} from ${color.blue(message.pushname)} ${message.isGroupMsg ? `in ${color.blue(message.groupMetadata.subject || "unknown")}` : ""}`.trim(), "info", message.timestamps))
  }
  ...
  if (message.body.startsWith("$") && isOwner) {
    new Promise<string>((resolve, reject) => {
      ...
      .catch((err: unknown) => message.reply(inspect(err, true)))
      .finally(() => client.log(`${color.purple("[EXEC]")} ${color.cyan(`$ [${arg.length}]`)} from ${color.blue(message.pushname)} ${message.isGroupMsg ? `in ${color.blue(message.groupMetadata.subject || "unknown")}` : ""}`.trim(), "info", message.timestamps))
  }
}
