...
export const execute = async (client: WAClient, message: GroupParticipantSerialize): Promise<unknown> => {
  const groupMetadata = (await database.getGroupMetadata(message.from)) ?? (await database.createGroupMetadata(message.from, (await client.groupMetadata(message.from)) as unknown))
  ...
  try {
    if (message.type === WAMessageStubType.GROUP_PARTICIPANT_ADD || message.type === WAMessageStubType.GROUP_PARTICIPANT_INVITE) {
      ...
      if (group.anticountry.active && !!groupMetadata.participants.find((member) => member.id === botNumber && member.admin)) {
        process.nextTick(() => message.body.forEach((participant) => group.anticountry.number.includes(phoneFormat(participant).countryCode) && client.groupParticipantsUpdate(message.from, [participant], "remove").catch(() => void 0)))
        await message.reply(
          i18n.translate(
