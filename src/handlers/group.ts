...
export const execute = async (client: WAClient, message: GroupSerialize): Promise<unknown> => {
  const group = (await database.getGroup(message.from)) ?? (await database.createGroup(message.from, { name: ((await database.getGroupMetadata(message.from)) ?? (await database.createGroupMetadata(message.from, (await client.groupMetadata(message.from)) as unknown))).subject }))
