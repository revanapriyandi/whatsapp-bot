...
export default <Command>{
  ...
  desc: "Landing menu",
  maintenance: false,
  execute: async ({ client, message, prefix, user, args, isOwner }) => {
    if (args.length === 1) {
      ...
      const text =
        "┏━━「 𓆩 𝐻ɪᴅᴅᴇɴ 𝐹ɪɴᴅᴇʀ ⁣𓆪 」\n" +
        "┃\n" +
        `┃ *${i18n.translate("commands.general.menu.cmd.one", {}, user.language)} :* ${cmd.aliases ? [name].concat(cmd.aliases).join(", ").trim() : name
        }\n` +
        ...
        `┃ *${i18n.translate("commands.general.menu.cmd.four", {}, user.language)} :* ${cmd.example
          ? cmd.example
