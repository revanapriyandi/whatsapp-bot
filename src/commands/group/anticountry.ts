...
export default <Command>{
  ...
  --------
  `.trimEnd(),
  execute: async ({ client, message, args, group, user, command, isBotGroupAdmin }) => {
      ...
      if (!group.anticountry.active) {
        ...
        process.nextTick(async () => {
          for (const participant of message.groupMetadata.participants.filter(
            ...
            if (isBotGroupAdmin) await client.groupParticipantsUpdate(message.from, [participant.id], "remove")
          }
        })
      }
